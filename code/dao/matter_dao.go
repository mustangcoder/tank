...
func (this *MatterDao) CheckWithRootByUuid(uuid string, user *model.User) *model.Matter {
	...
	if uuid == constant.MATTER_ROOT {
		if user == nil {
			panic(result.BadRequest("user cannot be null."))
		}
		matter = model.NewRootMatter(user)
	} else {
...
func (this *MatterDao) CheckWithRootByPath(path string, user *model.User) *model.Matter {
...
// find by path. if path=/, then return the Root Matter
func (this *MatterDao) FindWithRootByPath(path string, user *model.User) *model.Matter {
...
func (this *MatterDao) CheckByUuidAndUserUuid(uuid string, userUuid string) *model.Matter {
	...
	var matter = &model.Matter{}
	db := core.CONTEXT.GetDB().Where(&model.Matter{Base: model.Base{Uuid: uuid}, UserUuid: userUuid}).First(matter)
...
func (this *MatterDao) Page(page int, pageSize int, puuid string, userUuid string, name string, dir string, deleted string, extensions []string, sortArray []builder.OrderPair) *model.Pager {
...
func (this *MatterDao) TimesIncrement(matterUuid string) {
	db := core.CONTEXT.GetDB().Model(&model.Matter{}).Where("uuid = ?", matterUuid).Update(map[string]interface{}{"times": gorm.Expr("times + 1"), "visit_time": time.Now()})
	this.PanicError(db.Error)
}
...
func (this *MatterDao) SoftDelete(matter *model.Matter) {
	...
	//soft delete from db.
	db := core.CONTEXT.GetDB().Model(&model.Matter{}).Where("uuid = ?", matter.Uuid).Update(map[string]interface{}{"deleted": true, "delete_time": time.Now()})
	this.PanicError(db.Error)
...
func (this *MatterDao) Recovery(matter *model.Matter) {
	...
	//recovery from db.
	db := core.CONTEXT.GetDB().Model(&model.Matter{}).Where("uuid = ?", matter.Uuid).Update(map[string]interface{}{"deleted": false, "delete_time": time.Now()})
	this.PanicError(db.Error)
