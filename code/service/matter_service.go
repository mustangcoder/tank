...
func (this *MatterService) DownloadZip(
	...
	destZipDirPath := fmt.Sprintf("%s/%d", model.GetUserZipRootDir(matters[0].Username), time.Now().UnixNano()/1e6)
...
//delete files.
func (this *MatterService) Delete(request *http.Request, matter *model.Matter, user *model.User) {
...
//atomic delete files
func (this *MatterService) AtomicDelete(request *http.Request, matter *model.Matter, user *model.User) {
...
func (this *MatterService) AtomicSoftDelete(request *http.Request, matter *model.Matter, user *model.User) {
...
//upload files.
func (this *MatterService) Upload(request *http.Request, file io.Reader, user *model.User, dirMatter *model.Matter, filename string, privacy bool) *model.Matter {
	...
	if len(filename) > constant.MATTER_NAME_MAX_LENGTH {
		panic(result.BadRequestI18n(request, i18n.MatterNameLengthExceedLimit, len(filename), constant.MATTER_NAME_MAX_LENGTH))
	}
...
func (this *MatterService) createNonDirMatter(dirMatter *model.Matter, filename string, fileSize int64, privacy bool, user *model.User) *model.Matter {
...
// create a non dir matter.
func (this *MatterService) updateNonDirMatter(matter *model.Matter, fileSize int64, user *model.User) *model.Matter {
...
func (this *MatterService) ComputeRouteSize(matterUuid string, user *model.User) {

	//if to root directory, then update to user's info.
	if matterUuid == constant.MATTER_ROOT {

		size := this.matterDao.SizeByPuuidAndUserUuid(constant.MATTER_ROOT, user.Uuid)

		db := core.CONTEXT.GetDB().Model(&model.User{}).Where("uuid = ?", user.Uuid).Update("total_size", size)
...
func (this *MatterService) ComputeAllDirSize(user *model.User) {

	this.Logger.Info("Compute all dir's size for user %s %s", user.Uuid, user.Username)

	rootMatter := model.NewRootMatter(user)
	this.ComputeDirSize(rootMatter, user)
}

// compute a dir's size.
func (this *MatterService) ComputeDirSize(dirMatter *model.Matter, user *model.User) {
	...
	//if to root directory, then update to user's info.
	if dirMatter.Uuid == constant.MATTER_ROOT {
		...
		if dirMatter.Size != size {
			db := core.CONTEXT.GetDB().Model(&model.Matter{}).Where("uuid = ?", dirMatter.Uuid).Update("size", size)
			this.PanicError(db.Error)
		}
	}
...
func (this *MatterService) createDirectory(request *http.Request, dirMatter *model.Matter, name string, user *model.User) *model.Matter {
	...
	//if exist. return.
	matter := this.matterDao.FindByUserUuidAndPuuidAndDirAndName(user.Uuid, dirMatter.Uuid, constant.TRUE, name)
	if matter != nil {
		return matter
	}
	...
	if len(parts) > constant.MATTER_NAME_MAX_DEPTH {
		panic(result.BadRequestI18n(request, i18n.MatterDepthExceedLimit, len(parts), constant.MATTER_NAME_MAX_DEPTH))
	}

	absolutePath := model.GetUserMatterRootDir(user.Username) + dirMatter.Path + "/" + name
...
func (this *MatterService) AtomicCreateDirectory(request *http.Request, dirMatter *model.Matter, name string, user *model.User) *model.Matter {
...
func (this *MatterService) handleOverwrite(request *http.Request, user *model.User, destinationPath string, overwrite bool) {
...
func (this *MatterService) move(request *http.Request, srcMatter *model.Matter, destDirMatter *model.Matter, user *model.User) {
...
func (this *MatterService) AtomicMove(request *http.Request, srcMatter *model.Matter, destDirMatter *model.Matter, overwrite bool, user *model.User) {
...
func (this *MatterService) AtomicMoveBatch(request *http.Request, srcMatters []*model.Matter, destDirMatter *model.Matter, user *model.User) {
...
func (this *MatterService) AtomicCopy(request *http.Request, srcMatter *model.Matter, destDirMatter *model.Matter, name string, overwrite bool, user *model.User) {
...
func (this *MatterService) AtomicRename(request *http.Request, matter *model.Matter, name string, overwrite bool, user *model.User) {
...
//将本地文件映射到蓝眼云盘中去。
func (this *MatterService) AtomicMirror(request *http.Request, srcPath string, destPath string, overwrite bool, user *model.User) {
...
//将本地文件/文件夹映射到蓝眼云盘中去。
func (this *MatterService) mirror(request *http.Request, srcPath string, destDirMatter *model.Matter, overwrite bool, user *model.User) {
	...
	if fileStat.IsDir() {
		...
		//判断当前文件夹下，文件是否已经存在了。
		srcDirMatter := this.matterDao.FindByUserUuidAndPuuidAndDirAndName(user.Uuid, destDirMatter.Uuid, constant.TRUE, fileStat.Name())
		...
		//判断当前文件夹下，文件是否已经存在了。
		matter := this.matterDao.FindByUserUuidAndPuuidAndDirAndName(user.Uuid, destDirMatter.Uuid, constant.FALSE, fileStat.Name())
		if matter != nil {
			//如果是覆盖，那么删除之前的文件
			if overwrite {
				this.Delete(request, matter, user)
...
func (this *MatterService) CreateDirectories(request *http.Request, user *model.User, dirPath string) *model.Matter {

	dirPath = path.Clean(dirPath)

	if dirPath == "/" {
		return model.NewRootMatter(user)
	}
	...
	if len(folders) > constant.MATTER_NAME_MAX_DEPTH {
		panic(result.BadRequestI18n(request, i18n.MatterDepthExceedLimit, len(folders), constant.MATTER_NAME_MAX_DEPTH))
	}
	...
	for k, name := range folders {
		...
		if k == 0 {
			dirMatter = model.NewRootMatter(user)
			continue
		}
...
func (this *MatterService) WrapParentDetail(request *http.Request, matter *model.Matter) *model.Matter {
	...
	//when self not root.
	if matter.Uuid != constant.MATTER_ROOT {

		puuid := matter.Puuid
		tmpMatter := matter
		for puuid != constant.MATTER_ROOT {
...
func (this *MatterService) AtomicCrawl(request *http.Request, url string, filename string, user *model.User, dirMatter *model.Matter, privacy bool) *model.Matter {
...
func (this *MatterService) DeleteByPhysics(request *http.Request, user *model.User) {
...
func (this *MatterService) deleteFolderByPhysics(request *http.Request, dirMatter *model.Matter, user *model.User) {

	//scan user's file. scan level by level.
	this.matterDao.PageHandle(dirMatter.Uuid, user.Uuid, "", "", "", nil, nil, func(matter *model.Matter) {
...
func (this *MatterService) ScanPhysics(request *http.Request, user *model.User) {

	if user == nil {
		panic(result.BadRequest("user cannot be nil."))
	}

	rootDirPath := model.GetUserMatterRootDir(user.Username)
...
func (this *MatterService) scanPhysicsFolder(request *http.Request, dirInfo os.FileInfo, dirMatter *model.Matter, user *model.User) {
	if !dirInfo.IsDir() {
		return
	}
...
func (this *MatterService) CleanExpiredDeletedMatters() {
	...
	this.userDao.PageHandle("", "", func(user *model.User) {
		...
		this.matterDao.PageHandle("", "", "", constant.FALSE, constant.TRUE, &thenDate, nil, func(matter *model.Matter) {
			this.Delete(request, matter, user)
		})
		...
		this.matterDao.PageHandle("", "", "", constant.TRUE, constant.TRUE, &thenDate, sortArray, func(matter *model.Matter) {
			this.Delete(request, matter, user)
		})
